...
namespace Biofall.Gameplay.Mission1
{
    ...
    public sealed class ExtractionPoint : MonoBehaviour
    {
        ...
        /// <summary>Team extract: EVERY registered player must be on the pad (solo = the one player).</summary>
        private bool AllPlayersInZone(float radius)
        {
            ...
            {
                Transform p = all[i];
                if (p == null || (p.position - transform.position).sqrMagnitude > r2) return false;
            }
            return true;
        }
