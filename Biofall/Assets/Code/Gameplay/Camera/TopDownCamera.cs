...
namespace Biofall.Gameplay
{
    ...
    public sealed class TopDownCamera : MonoBehaviour
    {
        ...
        private void OnEnable() => EventBus.Subscribe<PlayerDamaged>(OnPlayerDamaged);
        private void OnDisable() => EventBus.Unsubscribe<PlayerDamaged>(OnPlayerDamaged);

        private void OnPlayerDamaged(PlayerDamaged e)
        {
            if (e.Amount > 0f) _shake = shakeAmplitude; // ignore the initial 0-damage sync
        }
