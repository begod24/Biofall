...
namespace Biofall.Gameplay
{
    ...
    public sealed class WaveSpawner : MonoBehaviour
    {
            ...
            while (true)
            {
                ...
                while (remZ + remS + remR + remT > 0)
                {
                    ...
                    if (mgr != null && mgr.ActiveCount < maxConcurrent)
                    {
                        ...
                        Vector3 pos = SnapToNavMesh(PickOffscreenPoint(center));
        ...
        private static Vector3 SnapToNavMesh(Vector3 p)
        {
            return NavMesh.SamplePosition(p, out NavMeshHit hit, 6f, NavMesh.AllAreas) ? hit.position : p;
        }
    }
}
