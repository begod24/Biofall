...
namespace Biofall.Core
{
    ...
    public static class PlayerRegistry
    {
        ...
        /// <summary>Players currently DOWNED/dead (co-op). They stay in <see cref="All"/> as bodies
        /// (teammates can find &amp; revive them) but are excluded from "is the squad here / alive"
        /// checks. Solo never marks anyone, so this is always empty there.</summary>
        ...
        /// <summary>Mark a player as downed/dead (co-op) or back up. See <see cref="_downed"/>.</summary>
        ...
        /// <summary>True if this player is currently downed/dead (co-op). Always false in solo.</summary>
        public static bool IsDowned(Transform player) => player != null && _downed.Contains(player);

        /// <summary>How many registered players are still up (not downed/dead). Solo = 1 while alive.</summary>
        ...
        /// <summary>Nearest player that is still UP (skips downed/dead bodies) — for enemy targeting
        /// that shouldn't swarm an incapacitated teammate. Falls back to null if everyone is down.</summary>
